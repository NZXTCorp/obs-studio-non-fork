...
static bool d3d11_shmem_init(HWND window)
{
	...
	static bool d3d11_shmem_init_buffers_logged = false;
	for (size_t i = 0; i < NUM_BUFFERS; i++) {
		if (!d3d11_shmem_init_buffers(i)) {
			if (!d3d11_shmem_init_buffers_logged) {
				hlog("d3d11_shmem_init: failed to init buffer %d", i);
				d3d11_shmem_init_buffers_logged = true;
			}
			return false;
		}
	}
	d3d11_shmem_init_buffers_logged = false;
...
{
	static bool d3d11_init_call_logged = false;
	if (!d3d11_init_call_logged) {
		hlog("d3d11_init called");
		d3d11_init_call_logged = true;
	}
	...
	if (!d3d11_init_format(swap, window)) {
		if (!logged_init_format_failure) {
			hlog("d3d11_init: failed to init format");
...
void d3d11_capture(void *swap_ptr, void *backbuffer_ptr)
{
	...
	if (!d3d11_capture_call_logged) {
		hlog("d3d11_capture called");
		d3d11_capture_call_logged = true;
	}
